...
const int togglePin[NUMBER_OF_TOGGLE_POSITIONS] = {2,3,4,5};        // the digital pin number for the SPDT toggle switch (connect center pin/pole to GND, throws to pins 2, 3, 4, 5)
const int servoPin[NUMBER_OF_SERVOS] = {6,7,8,9};                   // the digital pin number for the servo signal
const int potPin[NUMBER_OF_SERVOS] = {0,1,2,3};                     // the analog pin number for the pot signal (10K potentiometers)

const int toggleInterval[NUMBER_OF_TOGGLE_POSITIONS] = {200,200,200,200};  // number of millisecs between toggle readings
const int servoMinDegrees[NUMBER_OF_SERVOS] = {0,0,0,0};            // the limits to servo movement (0-180 nominal values)
const int servoMaxDegrees[NUMBER_OF_SERVOS] = {178,178,178,178};    // calibrate the values to your servos

Servo myservo[NUMBER_OF_SERVOS];                                    // create servo object to control a servo 

bool toggleState[NUMBER_OF_TOGGLE_POSITIONS] = {HIGH,HIGH,HIGH,HIGH};    // the current state of each toggle pin

int serialRead[2];                                                  // serial data from keyboard containing current pitch and note on/off value

int servoInterval[NUMBER_OF_SERVOS] = {15,15,15,15};                // initial millisecs between servo reads
int servoPosition[NUMBER_OF_SERVOS] = {90,90,90,90};                // the current angle of the servo - starting at 90 degrees
int servoDegrees[NUMBER_OF_SERVOS] = {3,3,3,3};                     // amount servo moves at each step, negative value moves opposite direction

int potInterval[NUMBER_OF_SERVOS] = {15,15,15,15};                  // initial millisecs between pot reads
...
void setup() {
 ...
 for (int i=0; i <NUMBER_OF_SERVOS; i++) {                          // iterate through each servo in array based on total servos number
   ...
   servoRandomMillis[i] = random(2000,20000);                          // pick a random time period between min & max number
 }
...
if (Serial1.available() >= 2){
 ...
 //servoRandomSweep(3);                                                // run auto random sweep function if proper toggle position is set to high
 servoRandomSweepMIDI(2);
...
void readPots() {
  ...
  for (int i=0; i <NUMBER_OF_SERVOS; i++) {
     if (currentMillis - previousPotMillis[i] >= potInterval[i]) {
        ...
        potValKnob[i] = map(potVal[i], 0, 1023, 5, 170);             // scale it to use it with the knob (position between 0-180 deg nominal)
        potValSweep[i] = map(potVal[i], 0, 1023, 5, 50);             // scale it to use it with the servo (slow to fast millisecs speed)
        potValMIDI[i] = map(potVal[i], 0, 1023, 1000, 5000);
    ...
    for (int i=0; i <NUMBER_OF_SERVOS; i++) {
      if (currentMillis - previousServoMillis[i] >= servoInterval[i]) {
       if (currentMillis > servoRandomMillis[i]) {                    // is it time to trigger a random direction change of the servo?
        servoDegrees[i] = - servoDegrees[i];                          // reverse direction
        servoRandomMillis[i] = random(0,10000) + currentMillis;       // pick new random time and add to current time for future trigger     
       }
       servoPosition[i] = servoPosition[i] + servoDegrees[i];         // servoDegrees might be negative  
       if ((servoPosition[i] >= servoMaxDegrees[i]) || (servoPosition[i] <= servoMinDegrees[i]))  { // if the servo is at either extreme change the sign of the degrees to make it move the other way           
         servoDegrees[i] = - servoDegrees[i];                         // reverse direction
